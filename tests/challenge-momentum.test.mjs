...
test('two clean challenge clears heal through the real adapter while campaign stays unchanged', () => {
  ...
  combat.start(0);
  combat.drainEvents();
  combat.playerHp = combat.playerMaxHp - 1;

  combat.events.push({ type: 'enemy-defeated', detail: { enemyId: combat.enemy.id, stage: 1 } });
  let events = combat.drainEvents();
  assert.equal(events.some((event) => event.type === 'challenge-rally'), false);
  assert.equal(combat.playerHp, combat.playerMaxHp - 1);

  combat.events.push({ type: 'stage-start', detail: { stage: 2, enemyId: 'wandering-ronin', enemyName: 'Wandering Ronin' } });
  combat.drainEvents();
  combat.events.push({ type: 'enemy-defeated', detail: { enemyId: 'wandering-ronin', stage: 2 } });
  ...
  assert.equal(rally?.detail?.cleanWaves, 2);
