...
await import('../src/perfect-step.js');
...
test('Perfect STEP auto riposte also triggers Blood Moon at six HP without posture damage', () => {
  const combat = new CombatEngine({ enemies: [BOSS_PHASE_ONE] });
  combat.start(0);
  combat.attackCursor = 1;
  combat.update(1550);
  combat.drainEvents();
  combat.enemyHp = 7;
  combat.enemyPosture = 0;

  const strikeAt = combat.phaseEndsAt;
  combat.update(strikeAt);
  assert.equal(combat.phase, 'strike');
  assert.equal(combat.currentAttack.direction, Direction.RIGHT);
  assert.equal(combat.snapshot(strikeAt).attack.reachDistance, 1);

  const result = combat.attemptBackstep(strikeAt);
  assert.equal(result.accepted, true);
  assert.equal(result.perfectStep, true);
  assert.equal(result.autoRiposteDamage, 1);
  assert.equal(result.bossPhase, 2);
  assert.equal(result.defeated, false);
  assert.equal(combat.enemyHp, 6);
  ...
  const events = combat.drainEvents();
  assert.ok(events.some((event) => event.type === 'counter' && event.detail?.automatic && event.detail?.perfectStep));
  assert.ok(events.some((event) => event.type === 'boss-phase' && event.detail?.phase === 2));
  assert.equal(events.some((event) => event.type === 'enemy-defeated'), false);

  const manual = combat.attemptAttack(Direction.LEFT, strikeAt + 20);
  assert.equal(manual.accepted, false);
  assert.equal(manual.reason, 'no-opening');
});

test('real app document loads perfect-step after its dependencies and before main runtime', async () => {
  const html = await readFile(new URL('../index.html', import.meta.url), 'utf8');
  const bossIndex = html.indexOf('./src/boss-overlay.js');
  const footworkIndex = html.indexOf('./src/footwork.js');
  const riposteIndex = html.indexOf('./src/perfect-riposte.js');
  const perfectStepIndex = html.indexOf('./src/perfect-step.js');
  const mainIndex = html.indexOf('./src/main.js');
  assert.ok(bossIndex >= 0, 'boss overlay module must be wired into the real app');
  assert.ok(footworkIndex > bossIndex, 'footwork must install after boss state is available');
  assert.ok(perfectStepIndex > riposteIndex, 'Perfect STEP must install after Perfect Parry event mapping');
  assert.ok(mainIndex > perfectStepIndex, 'Perfect STEP must install before main creates CombatEngine');
});
