...
- The release-prep guide is informational/navigation-only: it must not create a storage key, tester/player identifier, analytics event, network request or gameplay rule.
- The existing session-only 0/3 state may drive one compact **result-level 下一步** action that reuses already-existing controls only. After the first eligible campaign result it may launch an existing direct-practice route; during direct practice it may reuse the existing retry control to keep the same opponent; after `修行進度` reaches comparison it may open the existing explicit `回報` panel; at 3/3 it becomes a non-action completion receipt.
- The result-level 下一步 action is navigation only: it must not invent a new route, unlock, reward, gameplay rule, persistence key, identifier, analytics event or network path. It must remain hidden for `連戰試煉` / `今日陣`, because those terminal controls already own their replay flow.
- The 下一步 control must remain ≥44 px, fit the same 320×568 result viewport together with the existing `回報` and `分享` controls, and must not overlap either existing touch target.
- Acceptance for this result-level 下一步 flow must include a 320×568 Chromium path against the real production `index.html` composition, not only a synthetic DOM harness: it must reach a real campaign terminal, follow the real Ronin practice → same-opponent retry → `修行進度` comparison → existing feedback-panel path, and separately prove real `連戰試煉` and `今日陣` terminal results keep the action hidden.
- The same guide may show a **本機戰績** summary by read-only reuse of the existing campaign mastery best and challenge best records. It may derive a simple next-play suggestion from those already-stored values, but it must not create or update any storage key, add a hidden identifier, or imply cloud/global ranking.
