...
1. **Guard + four cuts + tempo self-inspection** — inspect exact Preview/runtime evidence at normal speed. Confirm the initial blade line visibly points at the player, refined Top/Bottom arcs and established RIGHT/LEFT cuts stay coherent through body/arms/Sword, measured/standard/quick rhythm reads match actual timing without overpowering the real blade, and full-blade afterimages remain subordinate. Refine only concrete defects; never return to per-frame Chest/arm/HandR overrides. Treat feint interpolation as a separate change only if exact runtime evidence shows the deliberate direct track switch remains visibly broken.
