...
- Runs 134–136: tested then rejected an unapproved structured feedback-topic expansion, restored the approved local/export-only payload, and reinstated the focused no-browser-persistence guard without changing runtime feedback behavior.
- Run 137: added campaign/direct-practice **敵式 / 應對** stage-intro profiles for Ashigaru, Ronin, Oni, Shogun and direct Blood Moon. The event-driven card clears before the first telegraph, stays pointer-transparent/in-bounds, is suppressed in challenge/今日陣 and changes no combat/persistence/network authority.
...
- Refine the new **敵式** stage-intro card only if exact Preview evidence shows it overlaps the existing stage intro, becomes repetitive, or teaches a mechanic inaccurately. Keep it pre-telegraph, campaign/direct-practice only and presentation-only rather than turning it into persistent live hints.
