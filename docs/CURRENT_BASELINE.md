...
- Challenge uses the same local mastery/run-analysis stream but stores a separate local challenge best, ranked first by waves cleared and then score. Challenge 氣勢/不屈 and 戰前抉擇 are run-local only; their HP/score effects are challenge-only and never read or overwrite the campaign personal best.
