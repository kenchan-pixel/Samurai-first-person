...
function draw(state, now, meta = {}) {
  view.draw(state, now, { attackDirectionIndex: 0, playerAction: 0, playerDirectionIndex: 0, hitAge: 999, shake: 0, ...meta });
  const impl = view.impl;
  const frame = { ...(impl.enemyPosturePresenceState || {}) };
  const recoil = { ...(impl.enemyParryRecoilState || {}) };
  const counter = { ...(impl.enemyCounterReactionState || {}) };
...
try {
  ...
  assert(!neutral.counter.active && !pressured.counter.active, 'Counter reaction leaked into neutral/high-posture gap');
  ...
  const telegraph = draw({ ...gap, phase: 'telegraph', phaseProgress: 0.55, enemyPosture: 5, attack: telegraphAttack }, 2300, { hitAge: 0.1, playerDirectionIndex: 1 });
  ...
  assert(!telegraph.counter.active && telegraph.counter.retreat === 0, 'Counter reaction leaked into live telegraph blade reading');
  ...
  assert(!baseRecovery.counter.active && !normalParry.counter.active && !perfectParry.counter.active, 'Counter reaction activated before a manual counter was consumed');
  ...
  assert(!broken.counter.active, 'Counter reaction activated before the guard-break counter landed');
  ...
  const counterAttack = { ...baseRecoveryAttack, parried: true, counterUsed: true };
  const counterBase = draw({ ...gap, phase: 'recovery', phaseProgress: 0.22, enemyPosture: 2, enemyHp: 5, attack: counterAttack }, 3000, { hitAge: 1.2, playerDirectionIndex: 1 });
  const counterHit = draw({ ...gap, phase: 'recovery', phaseProgress: 0.22, enemyPosture: 2, enemyHp: 4, attack: counterAttack }, 3000, { hitAge: 0.10, playerDirectionIndex: 1 });
  assert(!counterBase.counter.active && counterHit.counter.active, 'Accepted manual counter did not create a fresh whole-body hit reaction');
  assert(!counterHit.recoil.active, 'Pre-counter parry recoil did not clear when the manual counter was consumed');
  assert(counterHit.enemyZ < counterBase.enemyZ - 0.045, 'Manual counter did not visibly drive the enemy back at contact');
  assert(counterHit.enemyX > counterBase.enemyX + 0.025, 'RIGHT manual counter did not push the opponent in the swipe direction');
  assert(counterHit.swordParent === 'HandR' && counterHit.gripLocked && counterHit.orientationDeltaDeg < 0.25, 'Manual counter body reaction broke Sword/HandR grip authority');

  const counterBreakAttack = { ...counterAttack, perfect: true, guardBroken: true };
  const counterBreak = draw({ ...gap, phase: 'recovery', phaseProgress: 0.22, enemyPosture: 0, enemyHp: 2, attack: counterBreakAttack }, 3100, { hitAge: 0.10, playerDirectionIndex: 3 });
  assert(counterBreak.counter.active && counterBreak.counter.guardBroken, 'Guard-break manual counter did not activate its stronger body reaction');
  assert(counterBreak.counter.retreat > counterHit.counter.retreat, 'Guard-break counter reaction was not stronger than a normal counter');
  assert(counterBreak.counter.lateral < 0, 'LEFT manual counter did not mirror the directional body reaction');

  const finisher = draw({ ...gap, phase: 'stage-clear', phaseProgress: 0, enemyPosture: 0, enemyHp: 0, attack: counterBreakAttack }, 3200, { hitAge: 0.10, playerDirectionIndex: 2 });
  assert(finisher.counter.active && finisher.counter.defeated, 'Final manual counter did not enter the finisher body reaction');
  assert(finisher.counter.retreat > counterBreak.counter.retreat, 'Finisher body reaction was not stronger than guard-break counter impact');
  assert(finisher.counter.drop > counterBreak.counter.drop, 'Finisher body reaction did not add a visible collapse component');
  assert([finisher.enemyX, finisher.enemyY, finisher.enemyZ, finisher.characterPitch, finisher.characterYaw, finisher.characterRoll].every(Number.isFinite), 'Finisher counter reaction produced a non-finite transform');
  assert(finisher.swordParent === 'HandR' && finisher.gripLocked && finisher.orientationDeltaDeg < 0.25, 'Finisher body reaction broke Sword/HandR grip authority');

  const counterSettled = draw({ ...gap, phase: 'recovery', phaseProgress: 0.72, enemyPosture: 0, enemyHp: 2, attack: counterBreakAttack }, 3400, { hitAge: 1.08, playerDirectionIndex: 3 });
  assert(!counterSettled.counter.active && counterSettled.counter.retreat === 0 && counterSettled.counter.lateral === 0, 'Manual counter body reaction did not settle after its bounded impact age');

  root.dataset.enemyPostureRendererIntegration = 'pass';
  root.dataset.enemyPostureRendererViewport = '320x568';
  root.dataset.enemyPostureRendererSequence = 'neutral-pressure-telegraph-suppressed-parry-perfect-guard-break-counter-finisher-settle';
  root.dataset.enemyPostureRendererBlade = 'handr-grip-locked';
  root.dataset.enemyParryRecoilRenderer = 'normal-perfect-break';
  root.dataset.enemyCounterReactionRenderer = 'normal-break-finisher';
} catch (error) {
